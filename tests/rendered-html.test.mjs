...
test("ships the approved wedding photo and cool-neutral theme", async () => {
  ...
  assert.match(sourceConfig, /dateISO:\s*"2026-10-06T11:30:00\+08:00"/);
  assert.match(sourceConfig, /venue:\s*"中原油田宾馆（二所）"/);
  assert.match(sourceConfig, /time:\s*"10:00"[\s\S]*title:\s*"迎宾"[\s\S]*请您入席稍候/);
  assert.match(sourceConfig, /time:\s*"11:30"[\s\S]*title:\s*"仪式开始（草坪仪式）"/);
  assert.match(sourceConfig, /time:\s*"12:30"[\s\S]*title:\s*"婚礼午宴"/);
