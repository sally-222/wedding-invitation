...
test("ships the approved wedding photo and cool-neutral theme", async () => {
  ...
  assert.match(sourceConfig, /apiEndpoint:\s*"https:\/\/wedding-invitation-d8cw19676945d\.service\.tcloudbase\.com\/api\/seats"/);
  assert.match(sourceConfig, /胡阳/);
  assert.match(sourceConfig, /728416/);
  assert.match(sourceConfig, /韩旭/);
  assert.match(sourceConfig, /593827/);
  assert.match(sourceConfig, /apiEndpoint:\s*"https:\/\/wedding-invitation-d8cw19676945d\.service\.tcloudbase\.com\/api\/wishes"/);
